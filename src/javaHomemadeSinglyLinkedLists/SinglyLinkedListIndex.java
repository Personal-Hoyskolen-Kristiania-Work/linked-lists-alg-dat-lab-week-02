...
public class SinglyLinkedListIndex {
    // linked lists
    private static final SinglyLinkedList linkedListOne = new SinglyLinkedList(); //this bad boy can store so many integer values
    private static final SinglyLinkedList linkedListTwo = new SinglyLinkedList();
    private static final SinglyLinkedList dummyLinkedList = new SinglyLinkedList();
    // operations
    private static final SinglyInsertions insertion = new SinglyInsertions();
    private static final SinglyDeletions deletion = new SinglyDeletions();
    private static final SinglySearch searching = new SinglySearch();
    private static final PrintSinglyLinkedList print = new PrintSinglyLinkedList();
    ...
    private static void headInsertion(SinglyLinkedList list, int intVal) {
    ...
    private static void afterNodeWithKeyDataInsertion(SinglyLinkedList list, int key, int intVal) {
        list.head = insertion.insertAfterNodeValue(list.head, key, intVal);
    }

    // insert at position after selected nodes data variable value that equal to "key" value
    private static void atSpecifiedPositionInsertion(SinglyLinkedList list, int position, int intVal) {
        list.head = insertion.insertAtPosition(list.head, position, intVal);
    }

    // insert at tail position
    private static void tailInsertion(SinglyLinkedList list, int intVal) {
        list.head = insertion.tailInsertion(list.head, intVal);
    }

    // delete head node
    private static void deleteHeadNode(SinglyLinkedList list) {
        list.head = deletion.deleteAtHead(list.head);
    }

    // delete node at node with int data variable matching key value
    private static void deleteAtNode(SinglyLinkedList list, int key) {
        list.head = deletion.deleteAtSelectedNode(list.head, key);
    }

    // delete node at specified position in list
    private static void atSpecifiedPositionDeletion(SinglyLinkedList list, int position) {
    ...
    private static void searchForNode(SinglyLinkedList setList, SinglyLinkedList getList, int intVal) {
